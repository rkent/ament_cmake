...
def do_build_package(
  ...
  build_command = ['colcon', 'build',
    ...
    '--build-base', module_dir / 'build',
    '--install-base', module_dir / 'install']
  if build_options:
...
def test_from_template(module_dir):
  ...
  for options in additional_options:
    ...
    result = do_build_package(module_dir, options['name'], options, module_dir / 'packages')
    ...
    do_test_package(module_dir, options['name'], options)
