...
def do_build_package(package_name, packages_dir, module_dir, symlink_install=False):
  ...
  configure_command = [CMAKE_COMMAND, '-S', package_dir, '-B', build_dir, '--install-prefix', str(module_dir / 'install' / package_name)]
  if symlink_install:
...
def test_from_options(module_dir):
  ...
  for options in get_options():
    ...
    do_build_package(options['name'], generated_packages_dir, module_dir, options['symlink_install'])
...
def test_ament_python_test_package_with_overlay(module_dir) -> None:
    ...
    do_build_package(AMENT_PYTHON_TEST_PACKAGE_OVERLAY, SOURCE_DIR / 'test' / 'packages', module_dir)

    INSTALL_DIR = module_dir / 'install' / AMENT_PYTHON_TEST_PACKAGE_OVERLAY / PYTHON_INSTALL_DIR
    shutil.copytree(SOURCE_DIR / 'test' / 'packages' / AMENT_PYTHON_TEST_PACKAGE / AMENT_PYTHON_TEST_PACKAGE, compare_dir, dirs_exist_ok=True)
    shutil.copytree(SOURCE_DIR / 'test' / 'packages' / AMENT_PYTHON_TEST_PACKAGE_OVERLAY / AMENT_PYTHON_TEST_PACKAGE_OVERLAY, compare_dir, dirs_exist_ok=True)
